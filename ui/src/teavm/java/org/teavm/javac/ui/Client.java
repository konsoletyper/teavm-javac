...
public final class Client {
    ...
    private static void initStdout() {
        ...
        Window.current().onMessage((MessageEvent event) -> {
            var request = (FrameCommand) JSON.parse(((JSString) event.getData()).stringValue());
    ...
    private static void displayMarkInEditor(CompilerDiagnosticMessage request) {
        ...
        MarkOptions options = JSObjects.createWithoutProto();
        int gutterSeverity;
        switch (request.getSeverity()) {
            case "error":
    ...
    private static void executeCode(Int8Array code) {
        ...
        listener = event -> {
            var command = (FrameCommand) JSON.parse(((JSString) event.getData()).stringValue());
